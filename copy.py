...
CS_SOLUTIONS_FOLDER = "/Users/bhuang/ben-github/js-interview/leetcode/"
...
def copyJSSolutions():
  sourceFiles = [x for x in os.listdir(CS_SOLUTIONS_FOLDER) if x.endswith(".js")]
  for f in sourceFiles:
    solutionNum = f.split('_')[0]

    sourceFilePath = os.path.join(CS_SOLUTIONS_FOLDER, f) 

    targetFolder = getTargetFolder(solutionNum)
    if targetFolder:
      targetFiePath = os.path.join(targetFolder, "solution.js")
      print("Copy {} to {}".format(sourceFilePath, targetFiePath))
      copyfile(sourceFilePath, targetFiePath)
