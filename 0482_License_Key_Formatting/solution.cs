...
we will reinsert dashes. Additionally, all the lower case letters in the string must be converted 
to upper case.
...
{
    public string LicenseKeyFormatting_MemoryIssue(string S, int K)
    {
        S = S.Replace("-", "").ToUpper();
        int len = S.Length;
        if (len % K != 0)
            for (int i = 0; i < (K - (len % K)); i++)   // Do NOT use S.Length in for(;;)
            {
                S = '#' + S;
            }
        ...
        {
            group.Add(S.Substring(i, S.Length % K));
        }
